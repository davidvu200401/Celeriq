...
namespace Celeriq.DataCore.EFDAL
{
	...
	internal class LinqSQLFromClauseCollection : List<LinqSQLFromClause>
	{
		...
		internal string GetRealTableName(string parentTable, string field)
		{
			...
			string realTable = string.Empty;
			if (parentTable == "ConfigurationSetting") realTable = Celeriq.DataCore.EFDAL.Entity.ConfigurationSetting.GetTableFromFieldNameSqlMapping(field);
			else if (parentTable == "DimensionData") realTable = Celeriq.DataCore.EFDAL.Entity.DimensionData.GetTableFromFieldNameSqlMapping(field);
			else if (parentTable == "DimensionStore") realTable = Celeriq.DataCore.EFDAL.Entity.DimensionStore.GetTableFromFieldNameSqlMapping(field);
			else if (parentTable == "RepositoryActionType") realTable = Celeriq.DataCore.EFDAL.Entity.RepositoryActionType.GetTableFromFieldNameSqlMapping(field);
			else if (parentTable == "RepositoryData") realTable = Celeriq.DataCore.EFDAL.Entity.RepositoryData.GetTableFromFieldNameSqlMapping(field);
			else if (parentTable == "RepositoryDefinition") realTable = Celeriq.DataCore.EFDAL.Entity.RepositoryDefinition.GetTableFromFieldNameSqlMapping(field);
			else if (parentTable == "RepositoryLog") realTable = Celeriq.DataCore.EFDAL.Entity.RepositoryLog.GetTableFromFieldNameSqlMapping(field);
			else if (parentTable == "RepositoryStat") realTable = Celeriq.DataCore.EFDAL.Entity.RepositoryStat.GetTableFromFieldNameSqlMapping(field);
			else if (parentTable == "ServerStat") realTable = Celeriq.DataCore.EFDAL.Entity.ServerStat.GetTableFromFieldNameSqlMapping(field);
			else if (parentTable == "UserAccount") realTable = Celeriq.DataCore.EFDAL.Entity.UserAccount.GetTableFromFieldNameSqlMapping(field);
		...
		where K : class, nHydrate.EFCore.DataAccess.IBusinessObjectLINQQuery
	{
		...
		{
			var dc = new DataContext(Celeriq.DataCore.EFDAL.DBHelper.GetConnection());
		...
		public static IEnumerable<P> GetDistinct(
			...
			LinqSQLParser.ObjectTypeConstants objectType)
		{
			using (var dc = new DataContext(Celeriq.DataCore.EFDAL.DBHelper.GetConnection()))
			{
				var template = dc.GetTable<K>();
				var cmd = nHydrate.EFCore.DataAccess.BusinessEntityQuery.GetCommand<K, P>(dc, template, select, where);
				cmd.CommandTimeout = DEFAULTTIMEOUT;
			...
			)
		{
			return UpdateData(
				...
				getField,
				hasModifyAudit,
				Celeriq.DataCore.EFDAL.DataCoreEntities.GetConnectionString());
		}
			...
			)
		{
			...
			{
				var template = dc.GetTable<K>();
				using (var cmd = nHydrate.EFCore.DataAccess.BusinessEntityQuery.GetCommand<K, P>(dc, template, select, where))
				{
	...
	internal partial class LinqSQLParser
	{
		...
		private void RemapParentChild()
		{
			...
			foreach (var fromClause in _fromLinkList)
			{
				...
				if (_type == ObjectTypeConstants.Table)
				{
					...
					if (fromClause.TableName == "DimensionData")
					{
						childTables.Add(fromClause);
					}
					if (fromClause.TableName == "DimensionStore")
					{
						childTables.Add(fromClause);
					}
					...
					if (fromClause.TableName == "RepositoryData")
					{
						childTables.Add(fromClause);
					}
					if (fromClause.TableName == "RepositoryDefinition")
					{
						childTables.Add(fromClause);
					}
					...
					if (fromClause.TableName == "UserAccount")
					{
						childTables.Add(fromClause);
					}
				}
			...
			foreach (var field in _fieldList)
			{
				...
				if (_type == ObjectTypeConstants.Table)
				{
					if (clause.TableName == "ConfigurationSetting") realTable = Celeriq.DataCore.EFDAL.Entity.ConfigurationSetting.GetTableFromFieldAliasSqlMapping(field.Alias);
					else if (clause.TableName == "DimensionData") realTable = Celeriq.DataCore.EFDAL.Entity.DimensionData.GetTableFromFieldAliasSqlMapping(field.Alias);
					else if (clause.TableName == "DimensionStore") realTable = Celeriq.DataCore.EFDAL.Entity.DimensionStore.GetTableFromFieldAliasSqlMapping(field.Alias);
					else if (clause.TableName == "RepositoryActionType") realTable = Celeriq.DataCore.EFDAL.Entity.RepositoryActionType.GetTableFromFieldAliasSqlMapping(field.Alias);
					else if (clause.TableName == "RepositoryData") realTable = Celeriq.DataCore.EFDAL.Entity.RepositoryData.GetTableFromFieldAliasSqlMapping(field.Alias);
					else if (clause.TableName == "RepositoryDefinition") realTable = Celeriq.DataCore.EFDAL.Entity.RepositoryDefinition.GetTableFromFieldAliasSqlMapping(field.Alias);
					else if (clause.TableName == "RepositoryLog") realTable = Celeriq.DataCore.EFDAL.Entity.RepositoryLog.GetTableFromFieldAliasSqlMapping(field.Alias);
					else if (clause.TableName == "RepositoryStat") realTable = Celeriq.DataCore.EFDAL.Entity.RepositoryStat.GetTableFromFieldAliasSqlMapping(field.Alias);
					else if (clause.TableName == "ServerStat") realTable = Celeriq.DataCore.EFDAL.Entity.ServerStat.GetTableFromFieldAliasSqlMapping(field.Alias);
					else if (clause.TableName == "UserAccount") realTable = Celeriq.DataCore.EFDAL.Entity.UserAccount.GetTableFromFieldAliasSqlMapping(field.Alias);
				}
		...
		public static string GetRemappedLinqSql(LinqSQLFromClause tableInfo, string whereClause, LinqSQLFromClauseCollection fromLinkList, ObjectTypeConstants type)
		{
			if (type == ObjectTypeConstants.Table)
			{
				...
				{
					case "ConfigurationSetting": return Celeriq.DataCore.EFDAL.Entity.ConfigurationSetting.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
					case "DimensionData": return Celeriq.DataCore.EFDAL.Entity.DimensionData.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
					case "DimensionStore": return Celeriq.DataCore.EFDAL.Entity.DimensionStore.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
					case "RepositoryActionType": return Celeriq.DataCore.EFDAL.Entity.RepositoryActionType.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
					case "RepositoryData": return Celeriq.DataCore.EFDAL.Entity.RepositoryData.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
					case "RepositoryDefinition": return Celeriq.DataCore.EFDAL.Entity.RepositoryDefinition.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
					case "RepositoryLog": return Celeriq.DataCore.EFDAL.Entity.RepositoryLog.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
					case "RepositoryStat": return Celeriq.DataCore.EFDAL.Entity.RepositoryStat.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
					case "ServerStat": return Celeriq.DataCore.EFDAL.Entity.ServerStat.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
					case "UserAccount": return Celeriq.DataCore.EFDAL.Entity.UserAccount.GetRemappedLinqSql(whereClause, tableInfo.Alias, fromLinkList);
				}
			}
		...
		private void ParseSelect(string selectClause)
		{
			...
			foreach (string rawField in selectList)
			{
				...
				if (!hasAlias)
				{
					...
					{
						case "ConfigurationSetting": alias = Celeriq.DataCore.EFDAL.Entity.ConfigurationSetting.GetFieldAliasFromFieldNameSqlMapping(field); break;
						case "DimensionData": alias = Celeriq.DataCore.EFDAL.Entity.DimensionData.GetFieldAliasFromFieldNameSqlMapping(field); break;
						case "DimensionStore": alias = Celeriq.DataCore.EFDAL.Entity.DimensionStore.GetFieldAliasFromFieldNameSqlMapping(field); break;
						case "RepositoryActionType": alias = Celeriq.DataCore.EFDAL.Entity.RepositoryActionType.GetFieldAliasFromFieldNameSqlMapping(field); break;
						case "RepositoryData": alias = Celeriq.DataCore.EFDAL.Entity.RepositoryData.GetFieldAliasFromFieldNameSqlMapping(field); break;
						case "RepositoryDefinition": alias = Celeriq.DataCore.EFDAL.Entity.RepositoryDefinition.GetFieldAliasFromFieldNameSqlMapping(field); break;
						case "RepositoryLog": alias = Celeriq.DataCore.EFDAL.Entity.RepositoryLog.GetFieldAliasFromFieldNameSqlMapping(field); break;
						case "RepositoryStat": alias = Celeriq.DataCore.EFDAL.Entity.RepositoryStat.GetFieldAliasFromFieldNameSqlMapping(field); break;
						case "ServerStat": alias = Celeriq.DataCore.EFDAL.Entity.ServerStat.GetFieldAliasFromFieldNameSqlMapping(field); break;
						case "UserAccount": alias = Celeriq.DataCore.EFDAL.Entity.UserAccount.GetFieldAliasFromFieldNameSqlMapping(field); break;
					}
				}
