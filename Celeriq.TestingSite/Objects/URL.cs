...
namespace Celeriq.TestingSite.API
{
    /// <summary>
    ...
    public class URL : ICloneable
    {
            ...
            get
            {
                try
                {
                    ...
                    if (this.Parameters.Count != 0)
                    {
                        ...
                        {
                            if (!(_defaults.ContainsKey(item.Name.ToLower()) && _defaults[item.Name.ToLower()] == item.Value) && !string.IsNullOrEmpty(item.Value))
        ...
        {
            URLParameter param = Parameters.Find(x => x.Name == name);
            if (param == null)
            {
